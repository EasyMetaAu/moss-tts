#!/usr/bin/env python3
"""
Auto-scan voices/ directory and generate voices.jsonl
Usage: python3 scripts/gen-voices-jsonl.py [voices_dir] [output_jsonl]
"""
import json, sys
from pathlib import Path

VOICES_DIR = Path(sys.argv[1]) if len(sys.argv) > 1 else Path(__file__).parent.parent / "voices"
OUTPUT     = Path(sys.argv[2]) if len(sys.argv) > 2 else VOICES_DIR / "voices.jsonl"
